...
- `spfeastreport.command.*`
- `spfeastreport.review.*`
- `spfeastreport.*`
...
- `pending`
- `no_error`
- `banned`（已预留显示逻辑，后续接入实际封禁流程）

## 已完成的管理逻辑

- 默认只显示待处理记录
- 可切换为查看全部记录
- `No Error` 后，记录仍保留在 yml 中
- 但默认管理页不再显示该记录
...
```text
build/libs/SpFeastReport-1.0.0.jar
```
